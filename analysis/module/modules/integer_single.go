...
type handelFuncSingleSingle func(globalState *state.GlobalState)

type OverUnderflowAnnotationSingleSingle struct {
...
func (anno OverUnderflowAnnotationSingleSingle) Copy() OverUnderflowAnnotationSingleSingle {
	return OverUnderflowAnnotationSingleSingle{
...
func (anno OverUnderflowAnnotationSingleSingle) Translate(ctx *z3.Context) OverUnderflowAnnotationSingleSingle {
	return OverUnderflowAnnotationSingleSingle{
		OverflowingState: anno.OverflowingState,
		Satisfy: anno.Satisfy,
		Operator: anno.Operator,
...
type OverUnderflowStateAnnotationSingleSingle struct {
	OverflowingStateAnnotations []OverUnderflowAnnotationSingleSingle
}

func NewOverUnderflowStateAnnotationSingleSingle() *OverUnderflowStateAnnotationSingleSingle {
	return &OverUnderflowStateAnnotationSingleSingle{
		OverflowingStateAnnotations: make([]OverUnderflowAnnotationSingleSingle, 0),
	}
}
func (anno *OverUnderflowStateAnnotationSingleSingle) PersistToWorldState() bool {
	return false
}
func (anno *OverUnderflowStateAnnotationSingleSingle) PersistOverCalls() bool {
	return false
}
func (anno *OverUnderflowStateAnnotationSingleSingle) Copy() state.StateAnnotation {
	set := make([]OverUnderflowAnnotationSingleSingle, 0)
	for _, overflowAnno := range anno.OverflowingStateAnnotations {
		set = append(set, overflowAnno)
	}
	return &OverUnderflowStateAnnotationSingleSingle{
		OverflowingStateAnnotations: set,
	}
}
func (anno *OverUnderflowStateAnnotationSingleSingle) Translate(ctx *z3.Context) state.StateAnnotation {
	set := make([]OverUnderflowAnnotationSingleSingle, 0)
	for _, overflowAnno := range anno.OverflowingStateAnnotations {
		set = append(set, overflowAnno.Translate(ctx))
	}
	return &OverUnderflowStateAnnotationSingleSingle{
		OverflowingStateAnnotations: set,
	}
...
func isExistInSlice(value interface{}, arr interface{}) bool {
...
type IntegerArithmeticsSingleSingle struct {
	Name                 string
	SWCID                string
	Description          string
	PreHooks             []string
	...
	Cache                []int
	OstatesSatisfiable   []*state.GlobalState
	OstatesUnsatisfiable []*state.GlobalState
}

func NewIntegerArithmeticsSingleSingle() *IntegerArithmeticsSingleSingle {
	return &IntegerArithmeticsSingleSingle{
		...
		Cache:                make([]int, 0),
		OstatesSatisfiable:   make([]*state.GlobalState, 0),
		OstatesUnsatisfiable: make([]*state.GlobalState, 0),
	}
}

func (dm *IntegerArithmeticsSingleSingle) ResetModule() {
	dm.Issues = make([]*analysis.Issue, 0)
	dm.OstatesSatisfiable = make([]*state.GlobalState, 0)
	dm.OstatesUnsatisfiable = make([]*state.GlobalState, 0)
}

func (dm *IntegerArithmeticsSingleSingle) Execute(target *state.GlobalState) []*analysis.Issue {
...
func (dm *IntegerArithmeticsSingleSingle) AddIssue(issue *analysis.Issue) {
	dm.Issues = append(dm.Issues, issue)
}

func (dm *IntegerArithmeticsSingleSingle) GetIssues() []*analysis.Issue {
	return dm.Issues
}

func (dm *IntegerArithmeticsSingleSingle) GetPreHooks() []string {
	return dm.PreHooks
}

func (dm *IntegerArithmeticsSingleSingle) GetPostHooks() []string {
	return make([]string, 0)
}

func (dm *IntegerArithmeticsSingleSingle) GetCache() *utils.Set {
	return nil
}

func (dm *IntegerArithmeticsSingleSingle) _get_args(state *state.GlobalState) (*z3.Bitvec, *z3.Bitvec) {
...
func (dm *IntegerArithmeticsSingleSingle) _execute(globalState *state.GlobalState) []*analysis.Issue {

	address := getAddressFromStateSingleSingle(globalState)
	//if dm.Cache.Contains(address) {
	if isExistInSlice(address, dm.Cache){
		//return dm.Issues
		return nil
	}
	opcode := globalState.GetCurrentInstruction().OpCode.Name
	funcs := make(map[string][]handelFuncSingleSingle)
	funcs["ADD"] = []handelFuncSingleSingle{dm._handel_add}
	funcs["SUB"] = []handelFuncSingleSingle{dm._handel_sub}
	funcs["MUL"] = []handelFuncSingleSingle{dm._handel_mul}
	funcs["SSTORE"] = []handelFuncSingleSingle{dm._handel_sstore}
	funcs["JUMPI"] = []handelFuncSingleSingle{dm._handel_jumpi}
	funcs["CALL"] = []handelFuncSingleSingle{dm._handel_call}
	funcs["RETURN"] = []handelFuncSingleSingle{dm._handel_return, dm._handel_transaction_end}
	funcs["STOP"] = []handelFuncSingleSingle{dm._handel_transaction_end}
	funcs["EXP"] = []handelFuncSingleSingle{dm._handel_exp}
...
func (dm *IntegerArithmeticsSingleSingle) _handel_add(globalState *state.GlobalState) {
	//config := z3.GetConfig()
	...
	annotation := OverUnderflowAnnotationSingleSingle{
...
func (dm *IntegerArithmeticsSingleSingle) _handel_mul(globalState *state.GlobalState) {
	//config := z3.GetConfig()
	//ctx := z3.NewContext(config)
	//newState := globalState.Copy()
	//newState.Translate(ctx)
...
func (dm *IntegerArithmeticsSingleSingle) _handel_sub(globalState *state.GlobalState) {
	//config := z3.GetConfig()
	//ctx := z3.NewContext(config)
	//newState := globalState.Copy()
	//newState.Translate(ctx)
...
func (dm *IntegerArithmeticsSingleSingle) _handel_exp(globalState *state.GlobalState) {
	//config := z3.GetConfig()
...
func (dm *IntegerArithmeticsSingleSingle) _handel_sstore(globalState *state.GlobalState) {
	stack := globalState.Mstate.Stack
	value := stack.RawStack[stack.Length()-2]
	stateAnnotation := getOverflowUnderflowStateAnnotationSingleSingle(globalState)
	for _, annotation := range value.Annotations.Elements() {
		if reflect.TypeOf(annotation).String() == "modules.OverUnderflowAnnotationSingleSingle" {
			stateAnnotation.OverflowingStateAnnotations = append(stateAnnotation.OverflowingStateAnnotations, annotation.(OverUnderflowAnnotationSingleSingle))
		}
	}
}

func (dm *IntegerArithmeticsSingleSingle) _handel_jumpi(globalState *state.GlobalState) {
	stack := globalState.Mstate.Stack
	value := stack.RawStack[stack.Length()-2]
	stateAnnotation := getOverflowUnderflowStateAnnotationSingleSingle(globalState)
	for _, annotation := range value.Annotations.Elements() {
...
func (dm *IntegerArithmeticsSingleSingle) _handel_call(globalState *state.GlobalState) {
	stack := globalState.Mstate.Stack
...
func (dm *IntegerArithmeticsSingleSingle) _handel_return(globalState *state.GlobalState) {
	stack := globalState.Mstate.Stack
...
func (dm *IntegerArithmeticsSingleSingle) _handel_transaction_end(globalState *state.GlobalState) {
	stateAnnotation := getOverflowUnderflowStateAnnotationSingleSingle(globalState)
	for _, annotation := range stateAnnotation.OverflowingStateAnnotations {
		ostate := annotation.OverflowingState

		if isExistInSlice(ostate, dm.OstatesUnsatisfiable) {
			continue
		}

		if !isExistInSlice(ostate, dm.OstatesSatisfiable) {
			constraints := ostate.WorldState.Constraints.DeepCopy()
			constraints.Add(annotation.Constraint)
			_, sat := state.GetModel(constraints, nil, nil, false, globalState.Z3ctx)
			//sat := annotation.(OverUnderflowAnnotationSingleSingle).Satisfy
			if sat {
				fmt.Println("sat")
				//dm.OstatesSatisfiable.Add(ostate)
				dm.OstatesSatisfiable = append(dm.OstatesSatisfiable, ostate)
			} else {
				fmt.Println("unsat")
				//dm.OstatesUnsatisfiable.Add(ostate)
				dm.OstatesUnsatisfiable = append(dm.OstatesUnsatisfiable, ostate)
				continue
			}
		}
		...
		constraints := globalState.WorldState.Constraints.DeepCopy()
		//constraints.Add(annotation.(OverUnderflowAnnotationSingleSingle).Constraint.Translate(globalState.Z3ctx))
		constraints.Add(annotation.Constraint)
		...
		address := getAddressFromStateSingleSingle(ostate)
		//dm.Cache.Add(address)
		dm.Cache = append(dm.Cache, address)
		//dm.Issues.Append(issue)
		dm.Issues = append(dm.Issues, issue)
	}
}

func getAddressFromStateSingleSingle(globalState *state.GlobalState) int {
	return globalState.GetCurrentInstruction().Address
}

func getOverflowUnderflowStateAnnotationSingleSingle(globalState *state.GlobalState) *OverUnderflowStateAnnotationSingleSingle {
	typeInstance := &OverUnderflowStateAnnotationSingleSingle{}
	stateAnnotations := globalState.GetAnnotations(reflect.TypeOf(typeInstance))

	if len(stateAnnotations) == 0 {
		stateAnnotation := NewOverUnderflowStateAnnotationSingleSingle()
		globalState.Annotate(stateAnnotation)
		return stateAnnotation
	} else {
		return stateAnnotations[0].(*OverUnderflowStateAnnotationSingleSingle)
	}
}
*/
package modules
